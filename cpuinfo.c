...
inline static size_t read_sysfs_cache_size(const char* path) {
    ...
    if (fscanf(file, "%zu%c", &size, &suffix) > 0) {
        if (suffix == 'K' || suffix == 'k') size *= 1024;
                        ...
                        if(fscanf(f_type, "%31s", type)) { 
                            if(clevel == 1){
                                if (strcmp(type, "Data") == 0) {
                                    snprintf(path_size, sizeof(path_size), "/sys/devices/system/cpu/cpu%d/cache/index%d/size", cpu, index);
                                    size_t size = read_sysfs_cache_size(path_size);
                                    if (size > 0 && size < min_cachesize) min_cachesize = size;
                                }
                            }
                            if(clevel == 2){
